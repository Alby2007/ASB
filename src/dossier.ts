...
export async function gatherDossierInputs(
  ...
  for (const e of edges.slice(0, 8)) {
    edgeViews.push({ name: await memoryStore.displayNameFor(guildId, e.otherId), summary: e.summary, natures: e.natures, valence: e.valence, observations: e.observationCount });
  }
  ...
  for (const i of (ctx.interactions ?? []).slice(0, 5)) {
    interactionViews.push({ name: await memoryStore.displayNameFor(guildId, i.otherId), addressedCount: i.count });
  }
  if (edgeViews.length || interactionViews.length) {
    ...
    for (const o of observations.slice(0, 25)) {
      obsViews.push({ name: await memoryStore.displayNameFor(guildId, o.otherId), nature: o.nature, valence: o.valence, reason: o.reason, direction: o.direction });
    }
