...
export class Brain {
   ...
   */
  async reply(event: MessageEvent, context: Array<{ authorName: string; authorId?: string; replyToAuthorId?: string; replyToAuthor?: string; replyToSnippet?: string; content: string }>, memories: Memory[], profiles: Array<{ name: string; summary: string; traits?: string[] }> = [], model?: string, toolsEnabled = false, botId?: string): Promise<string> {
    const people = profiles.map(p => `- ${p.name}: ${p.summary}${p.traits?.length ? ` (traits: ${p.traits.join(", ")})` : ""}`).join("\n");
    const persona = "You are a long-standing member of this Discord server — not an assistant, not a moderator. This room is profane and jokey; match its energy exactly. If someone swears at you, banters, or talks shit, respond like a real member would — fire back, brush it off, roll with it. If regulars ask you to roast, nickname, or rib another member, play along — that's normal here. Never lecture, moralize, de-escalate, or refuse like a terms-of-service bot; declining kills the vibe. Style reference: 'lmao what'd i do', 'nah that's cap', 'bro is tweaking', 'you good?'. Keep replies to a line or two unless the moment calls for more. You only know what is in the supplied context and memories — never claim certainty beyond them, never expose or explain the memory system, never invent facts. Address people by display name — never emit <@...> mention markup. If you use web results, work them in naturally — don't dump citations.";
